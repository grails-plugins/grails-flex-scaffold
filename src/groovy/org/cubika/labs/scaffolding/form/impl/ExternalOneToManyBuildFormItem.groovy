...
/**
 * Extends AbstractRelationBuildFormItem adding external one to many builder functionality.
 *
...
class ExternalOneToManyBuildFormItem extends AbstractRelationBuildFormItem {
	 ...
	 * Constructor
	 */
	ExternalOneToManyBuildFormItem(property, classLoader) {
	...
	protected String buildFormItemComponent(binding) {
		...
		pw.println "				<${property.name}:${property.referencedDomainClass.shortName}ExternalOneToManyListView " +
	...
	protected void generateInnerViews(property) {
		...
		String classNameDir = "${nameDir}/${property.referencedDomainClass.shortName}ExternalOneToManyListView.mxml"
		String templateDir = FSU.resolveResources("/*" + antProp.'view.eotmlistfile').toString()
