...
class ExternalManyToOneBuildFormItem extends AbstractRelationBuildFormItem {
	 ...
	 * Constructor
	 */
	ExternalManyToOneBuildFormItem(property, classLoader) {
	...
	protected String buildFormItemComponent(binding) {
		...
		pw.println "				<${property.name}:${property.referencedDomainClass.shortName}ManyToOneView " +
	...
	protected void generateInnerViews(property) {
		...
		String classNameDir = "${nameDir}/${property.referencedDomainClass.shortName}ManyToOneView.mxml"
		String templateDir = FSU.resolveResources("/*" + antProp.'view.eotolistfile').toString()
