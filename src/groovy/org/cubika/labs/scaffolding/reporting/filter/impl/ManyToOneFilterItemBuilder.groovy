 ...
 */
class ManyToOneFilterItemBuilder extends StringFilterItemBuilder {
	 ...
	 */
	String build() {
		"""					<cubikalabs:ManyToOneFilterControl id="${prop.name}FilterItem" styleName="reportingStringFilters" mainLabel="{MultipleRM.getString(MultipleRM.localePrefix, '${prop.domainClass.propertyName}.${prop.name}')}" requestParameter="${propName}" textInputWidth="250"/>\n"""
	}
}
