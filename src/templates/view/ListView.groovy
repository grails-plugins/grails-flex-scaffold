...
%>
	...
	<mx:VBox styleName="listContainer" width="100%" height="100%">
		<cubikalabs:PaginationView width="100%" pageFilter="{_model.page}"
		                           event="{event.${domainClass.propertyName}.${className}GetPaginationEvent}" />
		<mx:Spacer height="0"/>
		<mx:DataGrid dataProvider="{_model.page.list}"
		             width="100%" height="100%" id="dg"
		             itemDoubleClick="edit()" doubleClickEnabled="true"
...
%>			</mx:columns>
	...
	<mx:HBox width="100%" includeInLayout="false" styleName="listContainerButtons" y="-2">
