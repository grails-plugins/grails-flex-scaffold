...
<% println BBBuilder.getMethods(domainClass)%>
		private function changeHandler():void {
			UIComponent(vs.selectedChild).setFocus();
		}
...
%>
	...
	<mx:ViewStack id="vs" selectedIndex="{_model.selectedIndexView}" change="changeHandler()"
		...
		<view:${className}ListView width="100%" height="100%" label="{MultipleRM.getString(MultipleRM.localePrefix, 'reporting.tabs.crud')}"
			hideEffect="fadeEff" showEffect="fadeEff"
