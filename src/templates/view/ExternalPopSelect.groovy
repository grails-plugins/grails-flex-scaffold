...
<mx:TitleWindow xmlns:mx="http://www.adobe.com/2006/mxml"
                ...
                showCloseButton="true" close="cancel()" title="${typeName}">
	...
	<mx:Script><![CDATA[
		...
		private function keyboardHandler(e:KeyboardEvent):void {
			...
			if (e.target.document.className == "PaginationLightView" &&
						e.keyCode != Keyboard.F8 && e.keyCode != Keyboard.F2) {
				e.preventDefault();
	...
	<mx:VBox paddingTop="2" paddingBottom="10"	paddingLeft="10" paddingRight="10">
		<cubikalabs:PaginationLightView width="100%" paddingBottom="10"
		                                pageFilter="{_model.externalpage}"
		                                event="{event.${domainClass.propertyName}.${className}ExternalGetPaginationEvent}" />

		<cubikalabs:CBKDataGrid dataProvider="{_model.externalpage.list}" allowMultipleSelection="{allowMultipleSelection}"
		                        id="dg" variableHeight="true" rowCount="8"
		                        doubleClickEnabled="true" doubleClick="okHandler()">
			<cubikalabs:columns>
<%
